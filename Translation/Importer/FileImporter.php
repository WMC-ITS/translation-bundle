...
class FileImporter
{
     ...
     * @param boolean                               $unConfirm   unconfirm trans unit ite
     * @return int
     */
    public function import(\Symfony\Component\Finder\SplFileInfo $file, $forceUpdate = false, $merge = false,$unConfirm=false)
    {
        ...
        foreach ($messageCatalogue->all($domain) as $key => $content) {
            ...
            if ($translation instanceof TranslationInterface) {
                ...
                if($unConfirm){
                    $translation = $this->transUnitManager->updateTranslation($transUnit, $locale, $content, false, true,\DateTime::createFromFormat("U", $file->getMTime()),false);
                }
                else{

                    $translation = $this->transUnitManager->updateTranslation($transUnit, $locale, $content, false, true,\DateTime::createFromFormat("U", $file->getMTime()));
                }

...
class TransUnitRepository extends EntityRepository
{
    ...
    public function getTransUnitList(array $locales = null, $rows = 20, $page = 1, array $filters = null)
    {
        ...
        $sortColumn = (isset($filters['sidx']) && in_array($filters['sidx'], ['domain', 'key', 'createdAt'])) ? $filters['sidx'] : 'id';
        $order = isset($filters['sord']) ? $filters['sord'] : 'ASC';

        $sortLocale = $sortKey = false;
        if(isset($filters['sidx'])&& $filters['sidx']!=$sortColumn){
            foreach ($locales as $locale) {
                if ($filters['sidx'] == $locale . '_updatedAt') {
                    $sortLocale = $locale;
                    $sortKey = 'updatedAt';
                }
            }
        ...
        if ($sortKey) {
            $builder = $this->createQueryBuilder('tu')
                ->leftJoin('tu.translations', 'te')
                ->andWhere('te.locale=:locale')
                ->setParameter('locale',$sortLocale)
                ->select('tu.id');
            $sortKey=sprintf('te.%s', $sortKey);
        } else {
            $builder = $this->createQueryBuilder('tu')
                ->select('tu.id');
            $sortKey=sprintf('tu.%s', $sortColumn);
        }

        $this->addTransUnitFilters($builder, $filters);
        $this->addTranslationFilter($builder, $locales, $filters);

        $ids = $builder->orderBy($sortKey, $order)
    ...
    protected function addTransUnitFilters(QueryBuilder $builder, array $filters = null)
    {
        if (isset($filters['_search']) && $filters['_search']) {
            ...
            if (!empty($filters['createdAt'])) {
                $builder->andWhere($builder->expr()->like('tu.createdAt', ':createdAt'))
                    ->setParameter('createdAt', sprintf('%%%s%%', $filters['createdAt']));
            }
        }
    }
    ...
    protected function addTranslationFilter(QueryBuilder $builder, array $locales = null, array $filters = null)
    {
        ...
        if (null !== $locales) {
            ...
            foreach ($locales as $locale) {
                ...
                if (isset($filters['confirmed']) && $filters['confirmed'] !== '0' && $filters['confirmed'] !== 0 && $filters['confirmed'] !== '') {
                    if ($locale . '_confirmed_1' === $filters['confirmed']) {
                        $confirmedLocate = $locale;
                        $confirmed = 1;
                    } elseif ($locale . '_confirmed_2' === $filters['confirmed']) {
                        $confirmedLocate = $locale;
                        $confirmed = 0;
                    } else {
                        //var_dump([$locale.'_confirmed_2'==$filters['confirmed'],$locale.'_confirmed_2',$filters['confirmed']]);
                    }
                } else {
                    //var_dump([$filters['confirmed'],$filters['confirmed']!='0',$filters['confirmed']!==0,$filters['confirmed']!='',isset($filters['confirmed'])]);
                }


            }
            if (!empty($confirmedLocate)) {
                if ($confirmed == 0) {
                    $qb->andWhere(
                        $qb->expr()->orX()
                            ->add($qb->expr()->eq('t.confirmed', ':confirmed_' . $confirmedLocate))
                            ->add('t.confirmed is null')
                    )
                        ->setParameter('confirmed_' . $confirmedLocate, $confirmed);


                } else {
                    $qb->andWhere($qb->expr()->eq('t.confirmed', ':confirmed_' . $confirmedLocate))
                        ->setParameter('confirmed_' . $confirmedLocate, $confirmed);
                }

                $qb->andWhere($qb->expr()->eq('t.locale', ':locale_' . $confirmedLocate))
                    ->setParameter('locale_' . $confirmedLocate, sprintf('%s', $confirmedLocate));
            }

            // var_dump([$locale,$filters[$locale.'_confirmed']]);

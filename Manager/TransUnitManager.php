...
class TransUnitManager implements TransUnitManagerInterface
{
    ...
    public function updateTranslation(TransUnitInterface $transUnit, $locale, $content, $flush = false, $merge = false, \DateTime $modifiedOn = null, $confirmed=null)
    {
        ...
        if ($found) {
            ...
            if($confirmed!==null){
                $translation->setConfirmed($confirmed);
            }
            $translation->setContent($content);
     ...
     * @return bool
     */
    public function removeTranslation(TransUnitInterface $transUnit, $locale)
    {
        $translation = null;
        $i = 0;
        $end = $transUnit->getTranslations()->count();
     ...
     * {@inheritdoc}
     */
    public function updateTranslationsContent(TransUnitInterface $transUnit, array $translations, $flush = false,array $confirmations,array $updatedAts)
    {

        foreach ($translations as $locale => $content) {
            $status=$confirmations[$locale]==='true'?true:false;
            $updatedAt=new \DateTime($updatedAts[$locale]===null??time());

            if (!empty($content) && $content!=='!'  && $content!=='null') {
                if ($transUnit->hasTranslation($locale)) {
                    $this->updateTranslation($transUnit, $locale, $content,false,false,null,$status,$updatedAt);

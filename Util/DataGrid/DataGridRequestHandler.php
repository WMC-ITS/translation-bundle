...
class DataGridRequestHandler
{
    ...
    public function updateFromRequest($id, Request $request)
    {
        ...
        $translationsContent=$translationsContentConfirmations =[];
        foreach ($this->localeManager->getLocales() as $locale) {
            $translationsContent[$locale] = urldecode($request->request->get($locale));
            $translationsContentConfirmations[$locale] = urldecode($request->request->get($locale.'_confirmed'));
            $translationsContentUpdatedAt[$locale] =urldecode( $request->request->get($locale.'_updatedAt'));
        }

        $this->transUnitManager->updateTranslationsContent($transUnit, $translationsContent,false,$translationsContentConfirmations,$translationsContentUpdatedAt);
            ...
            if (count($filters) > 0) {
                ...
                for ($i=0; $i<$end; $i++) {
                    ...
                    foreach ($filters as $column => $str) {
                        if (in_array($column, array('key', 'domain','createdAt'))) {

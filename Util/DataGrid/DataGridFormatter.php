...
class DataGridFormatter
{
    ...
    protected function formatOne($transUnit)
    {
        ...
        $formatted = array(
            ...
            '_key'    => $transUnit['key'],
            '_createdAt'    => $transUnit['createdAt']->format("d.m.Y\nG:i:s")
        );

        // add locales in the same order as in managed_locales param
        foreach ($this->localeManager->getLocales() as $locale) {
            $formatted[$locale] = null;
            $formatted[$locale.'_confirmed'] = null;
            $formatted[$locale.'_updatedAt'] = null;
        }
        ...
        foreach ($transUnit['translations'] as $translation) {
            if (in_array($translation['locale'], $this->localeManager->getLocales())) {
                ...
                $formatted[$translation['locale'].'_confirmed'] = $translation['confirmed'];
                $formatted[$translation['locale'].'_updatedAt'] = isset($translation['updatedAt'])?$translation['updatedAt']->format("d.m.Y\nG:i:s"):$currentDate->format("d.m.Y\nG:i:s");
            }
        }
    ...
    protected function toArray(TransUnitInterface $transUnit)
    {
        $data = array(
            ...
            'createdAt'          => $transUnit->getCreatedAt(),
            'translations' => array(),
        );
        ...
        foreach ($transUnit->getTranslations() as $translation) {
            $data['translations'][] = array(
                ...
                'confirmed' =>(bool) $translation->isConfirmed(),
            );
        }
